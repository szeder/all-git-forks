...
class GitExporter(object):
    def __init__(self, git_hg, repo, marksfile, gitmarksfile, prefix='refs/heads/'):
        self.git_hg = git_hg
        self.repo = repo
    ...
    def write_marks(self, base):
        path = os.path.join(base, self.marksfile)
    ...
    def setup(self, resume, base, use_options, use_features):
        self.use_options = use_options
        self.use_features = use_features
        self.option_quiet()

        self.feature_relative_marks()
        self.feature_export_marks(self.gitmarksfile)

        path = os.path.join(base, 'info', 'fast-import', self.gitmarksfile)

        if resume and os.path.exists(path):
            self.feature_import_marks(self.gitmarksfile)
            self.read_marks(base)

    def fast_export(self, base, resume=False, use_options=False, use_features=False):
        self.setup(resume, base, use_options, use_features)
