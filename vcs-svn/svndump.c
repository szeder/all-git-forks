...
static void init_keys(void)
{
	...
	/* version 2 format (Subversion 0.18.0) */
	...
	while ((t = buffer_read_line())) {
		...
		*val++ = '\0';
		...
		if (key == keys.svn_fs_dump_format_version) {
			...
			strlcpy(node_ctx.text_delta_base_md5,
				val, MD5_HEX_LENGTH + 1);
		} else if (key == keys.text_delta_base_sha1) {
			strlcpy(node_ctx.text_delta_base_sha1,
				val, SHA1_HEX_LENGTH + 1);
		} else if (key == keys.text_copy_source_sha1) {
			strlcpy(node_ctx.text_copy_source_sha1,
				val, SHA1_HEX_LENGTH + 1);
		} else if (key == keys.text_content_sha1) {
			strlcpy(node_ctx.text_content_sha1,
				val, SHA1_HEX_LENGTH + 1);
