...
static unsigned int strcmp_whitesp_ignore(const char *str1, const char *str2)
{
	...
	*p2 = '\0';
	...
	//if (rc != 0)
	//	tracef("cmp: %d\n'%s'\n'%s'\n'%s'\n'%s'", rc, hex_unprintable(buf1.buf), hex_unprintable(buf2.buf), hex_unprintable(o1), hex_unprintable(o2));
...
static void add_sort_cvs_revision(struct cvs_branch *meta, struct cvs_revision *rev)
{
	...
	if (!su->cur_file_path) {
		...
		if (!su->sorted) {
			...
			//tracef("%p sorted items %u adding %u file %s", meta, su->sorted->nr, su->cur_file->nr, su->cur_file->item[0]->path);
...
time_t find_first_commit_time(struct cvs_branch *meta)
{
	...
	for (i = 1; i < meta->rev_list->nr; i++) {
		//tracef("min search %s %s %s", meta->rev_list->item[i]->path,
...
static int print_cvs_revision(void *ptr, void *data)
{
	...
	if (rev->prev) {
		...
		while (prev && prev->ismerged && prev->prev)
			prev = prev->prev;
		if (prev->ismerged)
			strbuf_addf(&sb, "\tunknown->%s-", rev->prev->revision);
		else
			strbuf_addf(&sb, "\t%s->", rev->prev->revision);
	}
	else {
		strbuf_addstr(&sb, "\tunknown->");
	}
	strbuf_addf(&sb, "%s\t%s", rev->revision, rev->path);

	if (rev->isdead)
		strbuf_addstr(&sb, " (dead)");
...
void print_cvs_commit(struct cvs_commit *commit)
{
	...
	tracef("Author: %s", commit->author);
	tracef("AuthorDate: %s", show_date(commit->timestamp, 0, DATE_NORMAL));
	tracef("CommitDate: %s", show_date(commit->timestamp_last, 0, DATE_NORMAL));
	tracef("UpdateDate: %s", show_date(commit->cancellation_point, 0, DATE_NORMAL));
	tracef("%s", commit->msg);
...
void reverse_rev_list(struct cvs_revision_list *rev_list)
{
	...
	/*for (i = 0; i < rev_list->nr; i++) {
		tmp = rev_list->item[i];
		tracef("%p %s %s", tmp->cvs_commit, tmp->path, tmp->revision);
	}*/
}
