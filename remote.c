	...
	if (sent_tips.nr) {
		for_each_string_list_item(item, &src_tag) {
			...
			if (is_null_oid(&ref->new_oid))
				continue;
			commit = lookup_commit_reference_gently(ref->new_oid.hash, &error_silent);
			if (!commit)
			     ...
			     int force_update)
{
	...
	for (ref = remote_refs; ref; ref = ref->next) {
		...
		if (!reject_reason && !ref->deletion && !is_null_oid(&ref->old_oid)) {
			...
			else if (!lookup_commit_reference_gently(ref->old_oid.hash, &error_silent) ||
				 !lookup_commit_reference_gently(ref->new_oid.hash, &error_silent))
