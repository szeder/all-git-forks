...
static int seen_longer_match(enum http_option_type opt, size_t matchlen)
{
	/*
...
static int http_options(const char *var, const char *value, void *cb)
{
	...
	if (!strcmp("sslcert", key)) {
		if (seen_longer_match(opt_ssl_cert, matchlen))
			return 0;
		return git_config_string(&ssl_cert, var, value);
	}
#if LIBCURL_VERSION_NUM >= 0x070903
	if (!strcmp("sslkey", key)) {
		if (seen_longer_match(opt_ssl_key, matchlen))
			return 0;
	...
	if (!strcmp("sslcapath", key)) {
		if (seen_longer_match(opt_ssl_capath, matchlen))
			return 0;
		return git_config_string(&ssl_capath, var, value);
	}
#endif
	if (!strcmp("sslcainfo", key)) {
		if (seen_longer_match(opt_ssl_cainfo, matchlen))
			return 0;
		return git_config_string(&ssl_cainfo, var, value);
	}
	if (!strcmp("sslcertpasswordprotected", key)) {
		if (seen_longer_match(opt_passwd_req, matchlen))
			return 0;
		ssl_cert_password_required = git_config_bool(var, value);
		return 0;
	}
	if (!strcmp("ssltry", key)) {
		if (seen_longer_match(opt_ssl_try, matchlen))
			return 0;
		curl_ssl_try = git_config_bool(var, value);
		return 0;
	}
	if (!strcmp("minsessions", key)) {
		if (seen_longer_match(opt_min_sessions, matchlen))
			return 0;
	...
	if (!strcmp("maxrequests", key)) {
		if (seen_longer_match(opt_max_requests, matchlen))
			return 0;
	...
	if (!strcmp("lowspeedlimit", key)) {
		if (seen_longer_match(opt_low_speed, matchlen))
			return 0;
		curl_low_speed_limit = (long)git_config_int(var, value);
		return 0;
	}
	if (!strcmp("lowspeedtime", key)) {
		if (seen_longer_match(opt_low_time, matchlen))
			return 0;
	...
	if (!strcmp("noepsv", key)) {
		if (seen_longer_match(opt_no_epsv, matchlen))
			return 0;
		curl_ftp_no_epsv = git_config_bool(var, value);
		return 0;
	}
	if (!strcmp("proxy", key)) {
		if (seen_longer_match(opt_http_proxy, matchlen))
			return 0;
		return git_config_string(&curl_http_proxy, var, value);
	}

	if (!strcmp("cookiefile", key)) {
		if (seen_longer_match(opt_cookie_file, matchlen))
			return 0;
		return git_config_string(&curl_cookie_file, var, value);
	}

	if (!strcmp("postbuffer", key)) {
		if (seen_longer_match(opt_post_buffer, matchlen))
			return 0;
	...
	if (!strcmp("useragent", key)) {
		if (seen_longer_match(opt_user_agent, matchlen))
			return 0;
