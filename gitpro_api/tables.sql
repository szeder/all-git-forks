...
--TIME LOG TABLE
--HAS USER, TASK AND SPENT TIME (INIT DATE AND END DATE IN EPOCH FORMAT (time function in c))
CREATE TABLE IF NOT EXISTS GP_TIME_LOG (
	NOMBRE_USUARIO_LOG VARCHAR(100) NOT NULL,
	ID_LOG NUMERIC(10,0) NOT NULL,
	INICIO VARCHAR(100),
	FIN VARCHAR(100),
	CONSTRAINT LOG_PK PRIMARY KEY (NOMBRE_USUARIO_LOG,ID_LOG,INICIO,FIN),
	CONSTRAINT LOG_USUARIO_FK FOREIGN KEY (NOMBRE_USUARIO_LOG) REFERENCES GP_USUARIO(NOMBRE_USUARIO),
	CONSTRAINT LOG_TAREA_FK FOREIGN KEY (ID_LOG) REFERENCES GP_TAREA(ID)
);
