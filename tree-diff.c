	...
	int imin)
{
	...
	if (recurse) {
		...
		strbuf_add(base, path, pathlen);
		strbuf_addch(base, '/');
		p = ll_diff_tree_paths(p, sha1, parents_sha1, nparent, base, opt, pst);
			       ...
			       struct pathspec_trie *pst)
{
	...
	while (t->size) {
		match = pst ?
			...
			tree_entry_interesting(&t->entry, base, 0, &opt->pathspec);
		if (match) {
...
static void try_to_follow_renames(const unsigned char *old, const unsigned char *new, struct strbuf *base, struct diff_options *opt)
{
	...
	ll_diff_tree_sha1(old, new, base, &diff_opts, NULL);
			     ...
			     struct pathspec_trie *pst)
{
	...
	diff_tree_paths(&phead, new, &old, 1, base, opt, pst);
...
int diff_tree_sha1(const unsigned char *old, const unsigned char *new, const char *base_str, struct diff_options *opt)
{
	...
	retval = ll_diff_tree_sha1(old, new, &base, opt, *base_str ? NULL : opt->pathspec.trie);
