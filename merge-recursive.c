				      ...
				      const char *subtree_shift)
{
	...
	if (!*subtree_shift) {
		shift_tree(one->object.oid.hash, two->object.oid.hash, shifted, 0);
	} else {
		shift_tree_by(one->object.oid.hash, two->object.oid.hash, shifted,
			      subtree_shift);
	}
	if (!hashcmp(two->object.oid.hash, shifted))
		return two;
