...
def read_index_entries(f, header):
	...
	f.seek(indexlib.HEADER_V5_STRUCT.size
			...
			+ (header["ndir"] + 1) * indexlib.DIR_OFFSET_STRUCT.size
			+ indexlib.CRC_STRUCT.size)
	...
	files = dict()
	foffsetblock = f.tell() - (header["nfile"] + 1) * indexlib.DIR_OFFSET_STRUCT.size
	dirnr, fblockoffset, foffsetblock = read_files(f, directories, f.tell(),
			foffsetblock, 0, files)
	return files, fblockoffset


def read_file(f, pathname, fblockoffset, foffsetblock):
	...
	# simplify the code.
	crc.add(indexlib.FILE_OFFSET_STRUCT.pack(foffsetblock))

	filename = read_name(f, crc)

	(flags, mode, mtimes, mtimens, size, statcrc, objhash) = \
	...
	return dict(name=pathname + filename, flags=flags, mode=mode,
			mtimes=mtimes, mtimens=mtimens, size=size, statcrc=statcrc,
			foffsetblock=foffsetblock, objhash=binascii.hexlify(objhash),
			start=namestart)


def read_files(f, directories, fblockoffset, foffsetblock, dirnr, files_out):
	...
	for i in xrange(directories[dirnr]["nfiles"]):
		queue.append(read_file(f, directories[dirnr]["pathname"],
			fblockoffset, foffsetblock))
		foffsetblock += 4

	while queue:
		if (len(directories) > dirnr + 1 and
				queue[0]["name"] > directories[dirnr + 1]["pathname"]):
			dirnr, fblockoffset, foffsetblock = read_files(f,
					directories, fblockoffset, foffsetblock,
					dirnr + 1, files_out)
		else:
			new_file = queue.popleft()
			files_out[new_file["name"]] = new_file

	return dirnr, fblockoffset, foffsetblock
...
def update_index_entry(fw, filename, files, fblockoffset):
	...
	f = files[filename]
	fw.seek(f["start"])
	partialcrc = indexlib.calculate_crc(indexlib.OFFSET_STRUCT.pack(f["foffsetblock"]))
	partialcrc = write_calc_crc(fw, os.path.basename(filename) + "\0",
			partialcrc)

	data = indexlib.FILE_DATA_STRUCT.pack(f["flags"], f["mode"], f["mtimes"],
			f["mtimens"], f["size"]-1, f["statcrc"],
			binascii.unhexlify(f["objhash"]))
