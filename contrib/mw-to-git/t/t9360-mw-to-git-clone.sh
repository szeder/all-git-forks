...
test_expect_success 'Git clone creates only Main_Page.mw with an empty wiki' '
	wiki_reset &&
	git clone mediawiki::http://localhost/wiki mw_dir_3 &&
...
test_expect_success 'Git clone does not fetch a deleted page' '
	...
	git clone mediawiki::http://localhost/wiki mw_dir_4 &&
...
test_expect_success 'Git clone works with page added' '
	...
	git clone mediawiki::http://localhost/wiki mw_dir_5 &&
...
test_expect_success 'Git clone works with one specific page cloned ' '
	...
	git clone -c remote.origin.pages=namnam mediawiki::http://localhost/wiki mw_dir_8 &&
...
test_expect_success 'Git clone works with multiple specific page cloned ' '
	...
	wiki_delete_page namnam &&
	git clone -c remote.origin.pages='"'foo bar nyancat namnam'"' mediawiki::http://localhost/wiki mw_dir_9 &&
...
test_expect_success 'Mediawiki-clone of several specific pages on wiki' '
	...
	git clone -c remote.origin.pages="cloned_1 cloned_2 cloned_3" mediawiki::http://localhost/wiki mw_dir_10 &&
...
test_expect_success 'Git clone works with the shallow option' '
	...
	git -c remote.origin.shallow=true clone mediawiki::http://localhost/wiki/ mw_dir_11 &&
...
test_expect_success 'Git clone works with the shallow option with a delete page' '
	...
	wiki_editpage nyan "2nd revision, should be cloned" false &&
	wiki_delete_page foo &&
	git -c remote.origin.shallow=true clone mediawiki::http://localhost/wiki/ mw_dir_12 &&
...
test_expect_success 'Test of fetching a category' '
	...
	git clone -c remote.origin.categories="Category" mediawiki::http://localhost/wiki mw_dir_13 &&
...
test_expect_success 'Test of resistance to modification of category on wiki for clone' '
	...
	git clone -c remote.origin.categories="Catone" mediawiki::http://localhost/wiki mw_dir_14 &&
