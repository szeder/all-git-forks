...
test_expect_success 'git push can upload media (File:) files' '
	wiki_reset &&
	git clone mediawiki::http://'"$SERVER_ADDR:$PORT/$WIKI_DIR_NAME"' mw_dir &&
	(
		cd mw_dir &&
...
test_expect_success 'git clone works on previously created wiki with media files' '
	...
	git clone -c remote.origin.mediaimport=true \
		mediawiki::http://'"$SERVER_ADDR:$PORT/$WIKI_DIR_NAME"' mw_dir_clone &&
...
test_expect_success 'git clone works with one specific page and mediaimport=true' '
	git clone -c remote.origin.pages=testpage \
		  -c remote.origin.mediaimport=true \
			mediawiki::http://'"$SERVER_ADDR:$PORT/$WIKI_DIR_NAME"' mw_dir_15 &&
...
test_expect_success 'git clone works with one specific page cloned and mediaimport=false' '
	...
	git clone -c remote.origin.pages=testpage \
			mediawiki::http://'"$SERVER_ADDR:$PORT/$WIKI_DIR_NAME"' mw_dir_16 &&
...
test_expect_success 'git clone works with one specific page cloned and mediaimport unset' '
	...
	git clone -c remote.origin.pages=testpage \
		mediawiki::http://'"$SERVER_ADDR:$PORT/$WIKI_DIR_NAME"' mw_dir_17 &&
