...
{   int nh16_i; uint64_t th, tl;                                                  \
    rh = rl = 0;                                                             \
    for (nh16_i = 0; nh16_i < nw; nh16_i+= 2) {                              \
	MUL64(th,tl,get64PE((mp)nh16_+i)+(kp)[nh16_i],get64PE((mp)+nh16_i+1)+(kp)[nh16_i+1]);\
        ADD128(rh,rl,th,tl);                                                 \
    }                                                                        \
}
#define nh_16_2(mp, kp, nw, rh, rl, rh1, rl1)                                \
{   int nh162_i; uint64_t th, tl;                                            \
    rh1 = rl1 = rh = rl = 0;                                                 \
    for (nh162_i = 0; nh162_i < nw; nh162_i+= 2) {                           \
	MUL64(th,tl,get64PE((mp)+nh162_i)+(kp)[nh162_i],get64PE((mp)+nh162_i+1)+(kp)[nh162_i+1]);\
        ADD128(rh,rl,th,tl);                                                 \
	MUL64(th,tl,get64PE((mp)+nh162_i)+(kp)[nh162_i+2],get64PE((mp)+nh162_i+1)+(kp)[nh162_i+3]);\
...
{   int nvn_i; uint64_t th, tl;                                              \
    rh = rl = 0;                                                             \
    for (nvn_i = 0; nvn_i < nw; nvn_i+= 8) {                                 \
	MUL64(th,tl,get64PE((mp)+nvn_i  )+(kp)[nvn_i  ],get64PE((mp)+nvn_i+1)+(kp)[nvn_i+1]);\
	ADD128(rh,rl,th,tl);                                                 \
        MUL64(th,tl,get64PE((mp)+nvn_i+2)+(kp)[nvn_i+2],get64PE((mp)+nvn_i+3)+(kp)[nvn_i+3]);\
        ADD128(rh,rl,th,tl);                                                 \
        MUL64(th,tl,get64PE((mp)+nvn_i+4)+(kp)[nvn_i+4],get64PE((mp)+nvn_i+5)+(kp)[nvn_i+5]);\
        ADD128(rh,rl,th,tl);                                                 \
        MUL64(th,tl,get64PE((mp)+nvn_i+6)+(kp)[nvn_i+6],get64PE((mp)+nvn_i+7)+(kp)[nvn_i+7]);\
        ADD128(rh,rl,th,tl);                                                 \
    }                                                                        \
}
...
{   int nvn_i; uint64_t th, tl;                                                  \
    ...
    for (nvn_i = 0; nvn_i < nw; nvn_i+= 8) {                                             \
        MUL64(th,tl,get64PE((mp)+nvn_i  )+(kp)[nvn_i  ],get64PE((mp)+nvn_i+1)+(kp)[nvn_i+1]);\
        ADD128(rh,rl,th,tl);                                                 \
        MUL64(th,tl,get64PE((mp)+nvn_i  )+(kp)[nvn_i+2],get64PE((mp)+nvn_i+1)+(kp)[nvn_i+3]);\
        ADD128(rh1,rl1,th,tl);                                               \
        MUL64(th,tl,get64PE((mp)+nvn_i+2)+(kp)[nvn_i+2],get64PE((mp)+nvn_i+3)+(kp)[nvn_i+3]);\
        ADD128(rh,rl,th,tl);                                                 \
        MUL64(th,tl,get64PE((mp)+nvn_i+2)+(kp)[nvn_i+4],get64PE((mp)+nvn_i+3)+(kp)[nvn_i+5]);\
        ADD128(rh1,rl1,th,tl);                                               \
        MUL64(th,tl,get64PE((mp)+nvn_i+4)+(kp)[nvn_i+4],get64PE((mp)+nvn_i+5)+(kp)[nvn_i+5]);\
        ADD128(rh,rl,th,tl);                                                 \
        MUL64(th,tl,get64PE((mp)+nvn_i+4)+(kp)[nvn_i+6],get64PE((mp)+nvn_i+5)+(kp)[nvn_i+7]);\
        ADD128(rh1,rl1,th,tl);                                               \
        MUL64(th,tl,get64PE((mp)+nvn_i+6)+(kp)[nvn_i+6],get64PE((mp)+nvn_i+7)+(kp)[nvn_i+7]);\
        ADD128(rh,rl,th,tl);                                                 \
        MUL64(th,tl,get64PE((mp)+nvn_i+6)+(kp)[nvn_i+8],get64PE((mp)+nvn_i+7)+(kp)[nvn_i+9]);\
        ADD128(rh1,rl1,th,tl);                                               \
    }                                                                        \
}
#endif
...
{   uint64_t t1,t2,m1,m2,t;                                             \
    ...
    rh = rl = t = 0;                                                    \
    for (nh16_i = 0; nh16_i < nw; nh16_i+=2)  {                                        \
	t1  = get64PE(mp+nh16_i) + kp[nh16_i];                                    \
        t2  = get64PE(mp+nh16_i+1) + kp[nh16_i+1];                                \
        m2  = MUL32(t1 >> 32, t2);                                      \
        m1  = MUL32(t1, t2 >> 32);                                      \
...
void vhash_update(const unsigned char *m,
...
uint64_t xvhash(const unsigned char m[],
          unsigned int mbytes,
          uint64_t *tagl,
          vmac_ctx_t *ctx)
{
...
uint64_t vhash(const unsigned char m[],
          unsigned int mbytes,
          uint64_t *tagl,
          vmac_ctx_t *ctx)
{
...
uint64_t vmac(const unsigned char m[],
         unsigned int mbytes,
         unsigned char n[16],
         uint64_t *tagl,
         vmac_ctx_t *ctx)
