...
(function (gitweb) {
	 ...
	 *
	 * @param {String} Unique name of a cookie (letters, numbers, underscores).
	 * @param {String} The string value stored in a cookie.
	 ...
	 * used as cookie expires, or an integer specifying the expiration date from now
	 * on in days, or a Date object to be used as cookie expiration date.  If a
	 ...
	 * directory (default: path of page that created the cookie).
	 * @param {String} [options.domain] Override what web sites are allowed to
	 * access cookie (default: domain of page that created the cookie).
	 * @param {Boolean} [options.secure] If true, the secure attribute of the cookie
	 * will be set and the cookie would be accessible only from secure sites (cookie
	 * transmission will require secure protocol like HTTPS).
	 */
	gitweb.cookies = {
		 ...
		 */
		deleteCookie : function (sName, options) {
			options = options || {};
			options.expires = 'delete';

			this.setCookie(sName, '', options);
		}
		/* end of gitweb.cookies */
	};
