...
static int write_one_shallow(const struct commit_graft *graft, void *cb_data)
{
	...
	const char *hex = sha1_to_hex(graft->oid.sha1);
...
static int mark_uninteresting_oid(const char *refname,
			      ...
			      int flags, void *cb_data)
{
	return mark_uninteresting(refname, oid->sha1, flags, cb_data);
}
		   ...
		   const struct object_id *oid, int flags, void *cb_data)
{
	...
	ca->commits[ca->nr] = lookup_commit_reference_gently(oid->sha1, 1);
	if (ca->commits[ca->nr])
		ca->nr++;
	return 0;
}
				...
				int *ref_status)
{
	...
	head_ref((each_ref_fn *)add_ref, &ca);
