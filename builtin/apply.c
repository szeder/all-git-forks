...
int cmd_apply(int argc, const char **argv, const char *prefix_)
{
	...
	init_apply_state(&apply_state, prefix_);
	git_apply_config();
	...
	argc = parse_options(argc, argv, apply_state.prefix, builtin_apply_options,
			apply_usage, 0);
	...
	for (i = 0; i < argc; i++) {
		...
		if (!strcmp(arg, "-")) {
			errs |= apply_patch(&apply_state, 0, "<stdin>", options);
			read_stdin = 0;
			continue;
		} else if (0 < apply_state.prefix_length)
			arg = prefix_filename(apply_state.prefix,
					      apply_state.prefix_length,
					      arg);
		...
		errs |= apply_patch(&apply_state, fd, arg, options);
		close(fd);
	}
	set_default_whitespace_mode(whitespace_option);
	if (read_stdin)
