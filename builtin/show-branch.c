...
static int append_head_ref_oid(const char *refname, const struct object_id *oid, int flag, void *cb_data)
{
	return append_head_ref(refname, oid->sha1, flag, cb_data);
}
...
static int append_remote_ref(const char *refname, const struct object_id *oid, int flag, void *cb_data)
{
	 ...
	 * get confused.
	 */
	if (get_sha1(refname + ofs, tmp) || hashcmp(tmp, oid->sha1))
		ofs = 5;
	return append_ref(refname + ofs, oid->sha1, 0);
}
...
static int append_matching_ref(const char *refname, const struct object_id *oid, int flag, void *cb_data)
{
	...
	if (starts_with(refname, "refs/heads/"))
		return append_head_ref(refname, oid->sha1, flag, cb_data);
	if (starts_with(refname, "refs/tags/"))
		return append_tag_ref(refname, oid->sha1, flag, cb_data);
	return append_ref(refname, oid->sha1, 0);
}
...
static void snarf_refs(int head, int remotes)
{
	if (head) {
		...
		for_each_ref(append_head_ref_oid, NULL);
