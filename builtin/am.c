...
{
	struct object_id orig_tree, his_tree, our_tree;

	if (get_sha1("HEAD", our_tree.hash) < 0)
	...
	if (write_index_as_tree(orig_tree.hash,
			&the_index, index_path, 0, NULL))
	...
	if (write_index_as_tree(his_tree.hash, &the_index, index_path, 0, NULL))
	...
	if (run_fallback_merge_recursive(state,
