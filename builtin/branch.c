	...
	for (i = 0; i < nr_branches; ++i) {
		...
		if (branches[i]->remote_name &&
			...
			if (found_merge_with_old_name) {
				char *key_to_change;
				key_to_change = xmalloc(strlen("branch.") +
										strlen(branches[i]->name) +
										strlen(".merge") + 1);

				strcpy(key_to_change, "branch.");
				strcpy(key_to_change + 7, branches[i]->name);
				strcpy(key_to_change + 7 + strlen(branches[i]->name), ".merge");

				git_config_set_multivar(key_to_change, newref.buf, oldref.buf, 1);
			}
		}
	}
