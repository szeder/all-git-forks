...
static int add_from_revs(struct blame_tree *bt)
{
	...
	for (i = 0; i < bt->rev.pending.nr; i++) {
		...
		diff_tree_sha1(EMPTY_TREE_SHA1_BIN, obj->item->sha1, "", &diffopt);
		diff_flush(&diffopt);
	}
...
static void mark_path(const char *path, const unsigned char *sha1,
			 ...
			 struct diff_options *opt, void *cbdata)
{
	...
	for (i = 0; i < q->nr; i++) {
		...
		default:
			 ...
			 */
			mark_path(p->two->path, p->two->sha1, data);
			break;
		}
	}
}
