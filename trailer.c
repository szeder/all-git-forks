...
static int alnum_len(const char *buf, int len)
{
...
static void apply_arg_if_exists(struct trailer_item *infile_tok,
				struct trailer_item *arg_tok,
				int alnum_len)
{
			       ...
			       enum action_where where)
{
	...
	for (arg_tok = *arg_tok_first; arg_tok; arg_tok = next_arg) {
		...
		if (same_token(infile_tok, arg_tok, tok_alnum_len) &&
			...
			apply_arg_if_exists(infile_tok, arg_tok, tok_alnum_len);
			/*
