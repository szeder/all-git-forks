			     ...
			     unsigned long *size)
{
	...
	hash = get_note(t, obj_hash);
	if (!hash)
		return NULL;

	note = read_sha1_file(hash, &type, size);
	if (type != OBJ_BLOB) {
		free(note);
		note = NULL;
	}
...
static int rename_by_commit(struct diff_options *opt)
{
	...
	free(last_note);
	hash = opt->current_commit->object.oid.hash;
	last_note = get_rename_note(opt->rename_notes, hash, &size);
