 ...
 * 1: @, look for a following { to reject @{ in refs (in non-SIMD, like 0)
 * 2: \0: End-of-component and string
 * 3: /: End-of-component (SIMD or = 2)
 * 4: ., look for a preceding . or / to reject .. in refs or /.
 * 5: {, look for a preceding @ to reject @{ in refs
 * 6: *, usually a bad character except once as a wildcard (in non-SIMD, like 7)
...
int check_refname_format_bytewise(const char *refname, int flags)
{
	...
	while(1) {
		...
		if (mask) {
			...
			case 3:
				component_count ++;
				/*
				 * don't allow components starting with '.'
				 */
				if (cp[i + 1] == '.') {
					return -1;
