				   ...
				   int flags, void *cb_data)
{
	...
	resolves_to = resolve_ref_unsafe(refname, junk.sha1, 0, NULL);
...
static int do_head_ref(const char *submodule, each_ref_fn fn, void *cb_data)
{
	...
	if (submodule) {
		if (resolve_gitlink_ref(submodule, "HEAD", oid.sha1) == 0)
			return fn("HEAD", &oid, 0, cb_data);

		return 0;
	}

	if (!read_ref_full("HEAD", oid.sha1, 1, &flag))
...
int head_ref_namespaced(each_ref_fn fn, void *cb_data)
{
	...
	if (!read_ref_full(buf.buf, oid.sha1, 1, &flag))
	...
	while ((de = readdir(d)) != NULL) {
		...
		if (stat(git_path("logs/%s", name->buf), &st) < 0) {
			...
			if (S_ISDIR(st.st_mode)) {
				...
				struct object_id oid;
				if (read_ref_full(name->buf, oid.sha1, 0, NULL))
