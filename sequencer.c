...
static int run_git_commit(const char *defmsg, struct replay_opts *opts, int empty)
{
	...
	if (!empty && !opts->keep_redundant_commits && index_unchanged)
		/*
		 * The head tree and the index match, meaning the
		 * commit is empty.  Since it wasn't created empty
		 * (based on the previous test), we can conclude the
		 * commit has been made redundant.  Since we don't
