...
{
	struct object *o = lookup_unknown_object(oid->sha1);
	...
	if (!o)
		die("git upload-pack: cannot find object %s:", sha1_to_hex(oid->sha1));
	o->flags |= OUR_REF;
	return 0;
}
...
static int send_ref(const char *refname, const struct object_id *oid, int flag, void *cb_data)
{
	...
	if (capabilities) {
		...
		packet_write(1, "%s %s%c%s%s%s%s agent=%s\n",
			     sha1_to_hex(oid->sha1), refname_nons,
			     0, capabilities,
	...
	capabilities = NULL;
	if (!peel_ref(refname, peeled.sha1))
		packet_write(1, "%s %s^{}\n", sha1_to_hex(peeled.sha1), refname_nons);
	return 0;
}

static int find_symref(const char *refname, const struct object_id *oid, int flag,
		       void *cb_data)
{
