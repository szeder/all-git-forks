 ...
 *
 *    An attempt is made to resolve any symbolic links in $FILENAME
 *    before creating the lockfile, so that the lockfile is next to
 *    the ultimate file that is being locked.
 *
 ...
 *   the file.
 *
 * - Locked, staging file closed and committed (after
 *   commit_staging_file()).  Same as the previous state, except that
 *   the separate staging file has already been closed and renamed on
 *   top of the file, staging_filename is the empty string, and fd is
 *   -1.
 *
...
/* A separate staging file is in being used */
...
int commit_staging_file(struct lock_file *lk)
{
	int err;
