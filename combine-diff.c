		       ...
		       const char *path)
{
	...
	if (S_ISGITLINK(mode)) {
		...
		*size = fill_textconv(textconv, df, &blob);
		...
		if (type != OBJ_BLOB)
			die("object '%s' is not a blob!", sha1_to_hex(oid->sha1));
	}
	return blob;
}
					   ...
					   int num_parent)
{
	...
	for (i = 0; i < num_parent; i++) {
		...
		pair->one[i].sha1_valid = !is_null_sha1(p->parent[i].oid.sha1);
	...
	pair->two->sha1_valid = !is_null_sha1(p->oid.sha1);
	return pair;
}
