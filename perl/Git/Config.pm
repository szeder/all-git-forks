...
sub _config {
	...
	my $_read = "read_${which}state";
	my $_state = "${which}state";

	if (!$self->{$_read}) {
		$self->read;
	}

	my $state = $self->{$_state}{$item};
...
sub global { return $_[0]->_config("global_", @_[1..$#_]) }
sub system { return $_[0]->_config("system_", @_[1..$#_]) }
