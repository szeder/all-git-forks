...
#     the file.
# (4) If data with the pathname "reject-all.r" is processed with any
#     operation then the filter signals that it does not want to process
#     the file and any file after that is processed with the same command.
#
...
while (1) {
    ...
    my $output;
    if ( $pathname eq "reject.r" or $pathname eq "reject-all.r" ) {
        $output = "";
    }
    ...
    if ( $pathname eq "reject.r" ) {
        print $debug "[REJECT]\n";
        $debug->flush();
        packet_write("status=reject\n");
    }
    elsif ( $pathname eq "reject-all.r" ) {
        print $debug "[REJECT-ALL]\n";
        $debug->flush();
        packet_write("status=reject-all\n");
    }
    else {
        packet_write("status=accept\n");
