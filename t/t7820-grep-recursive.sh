...
test_description='git grep --recurse-submodules test

This test checks the ability of git grep to search within submodules when told
to do so with the --recurse-submodules option'
...
test_expect_success 'recursive grep (with --or)' '
	...
	t:four five six
	EOF
	git grep --recurse-submodules \( -e "five" --or -e "two" \) >actual &&
	test_cmp expected actual
'
...
test_expect_success 'recursive grep (with --and --not)' '
	...
	t:four five six
	EOF
	git grep --recurse-submodules \( -e "five" --and --not -e "#1" \) >actual &&
	test_cmp expected actual
'
...
test_expect_success 'recursive grep with refspec' '
	...
	EOF
	git grep --recurse-submodules five master >actual &&
	test_cmp expected actual
'
...
test_expect_success 'recursive grep with pathspec' '
	...
	submodule2/t:four five six #2
	EOF
	git grep --recurse-submodules five -- submodule2 >actual &&
	test_cmp expected actual
'
...
test_expect_success 'recursive grep with pathspec and refspec' '
	...
	master:submodule2/t:four five six #2
	EOF
	git grep --recurse-submodules five master -- submodule2 >actual &&
	test_cmp expected actual
'
...
test_expect_failure 'recursive grep with --max-depth' '
	...
	t:four five six
	EOF
	git grep --recurse-submodules --max-depth=1 five  >actual &&
	test_cmp expected actual
'
test_done
