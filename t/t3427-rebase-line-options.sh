...
test_expect_success 'squash --signoff' '
	git checkout -b squash--signoff master &&
	cat >expected.msg <<-EOF &&
	$(commit_message HEAD^)
	...
	EOF
	commit_authorship HEAD^ >expected.author &&
	test_msg_author squash_--signoff
'

test_expect_success 'fixup --signoff' '
	git checkout -b fixup--signoff master &&
	cat >expected.msg <<-EOF &&
	$(commit_message HEAD^)
	...
	EOF
	commit_authorship HEAD^ >expected.author &&
	test_msg_author fixup_--signoff
'
...
test_expect_success 'squash --reset-author' '
	git checkout -b squash--reset-author master &&
	cat >expected.msg <<-EOF &&
	$(commit_message HEAD^)
	...
	commit_authorship HEAD^ >original.author &&
	test_diff_file expected.author original.author &&
	test_msg_author squash_--reset-author
'

test_expect_success 'fixup --reset-author' '
	git checkout -b fixup--reset-author master &&
	commit_message HEAD^ >expected.msg &&
	test_tick &&
	authorship >expected.author &&
	commit_authorship HEAD^ >original.author &&
	test_diff_file expected.author original.author &&
	test_msg_author fixup_--reset-author
'
