...
test_expect_success PERL 'process filter should not restart after intentionally rejected file' '
	...
	(
		...
		check_rot13 ../test.o test.r &&
		check_rot13 ../test2.o test2.r &&
		test_cmp reject.o reject.r
	)
'

test_expect_success PERL 'process filter should be able to reject this and all future blobs' '
	test_config_global filter.protocol.process "$TEST_DIRECTORY/t0021/rot13-filter.pl clean smudge" &&
	rm -rf repo &&
	mkdir repo &&
	(
		...
		cp ../test.o test.r &&
		cp ../test2.o test2.r &&
		echo "reject this blob and all future blobs" >reject-all.o &&
		cp reject-all.o reject-all.r &&
		git add . &&
		git commit . -m "test commit" &&
		rm -f *.r &&
		...
		check_filter_ignore_clean \
			git checkout . \
				<<-\EOF &&
					start
					wrote filter header
					IN: smudge reject-all.r 38 [OK] -- OUT: 0 [REJECT-ALL]
				EOF

		test_cmp ../test.o test.r &&
		test_cmp ../test2.o test2.r &&
		test_cmp reject-all.o reject-all.r
	)
'
