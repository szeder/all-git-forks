...
svnrepo=$PWD/httpd/svnrepo
export svnrepo
svnconf=$PWD/httpd/svnconf
export svnconf
...
if test -n "$SVN_HTTPD_PATH"
then
	LIB_HTTPD_PATH="$SVN_HTTPD_PATH"
else
	unset LIB_HTTPD_PATH
fi

if test -n "$SVN_HTTPD_PORT"
then
	LIB_HTTPD_PORT="$SVN_HTTPD_PORT"
else
	unset LIB_HTTPD_PORT
fi

# Start httpd with SVN in case lib-httpd starts it at all
LIB_HTTPD_SVN=t
