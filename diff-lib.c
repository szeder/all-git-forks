...
static int fs_cache_check_removed(const struct cache_entry *ce, struct stat *st)
{
	struct fsc_entry *fe;

	fe = fs_cache_file_exists(ce->name, ce_namelen(ce));
	if (!fe) {
		return 1;
	}
	if (fe_deleted(fe)) {
...
static int check_removed(const struct cache_entry *ce, struct stat *st)
{
	if (fs_cache_is_valid())
		return fs_cache_check_removed(ce, st);
