                                   ...
                                   const int wanted_capability)
{
	...
	filter_status = packet_read_line(process->out, NULL);

	if (filter_status && !strcmp(filter_status, "status=accept")) {
	...
	if (!filter_status || strcmp(filter_status, "status=success"))
		err = -1;
	...
	if (err || errno == EPIPE) {
		if (filter_status && !strcmp(filter_status, "status=reject-all")) {
			/*
		...
		} else if (!filter_status || strcmp(filter_status, "status=reject")) {
			/*
