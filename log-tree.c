				   ...
				   unsigned long *size)
{
	...
	data = read_sha1_file(entry->oid.hash, &type, size);
	if (type != OBJ_BLOB)
...
static int do_diff_remerge(struct rev_info *opt, struct commit *commit)
{
	...
	branch1 = xstrdup(oid_to_hex(&parent1->object.oid));
	branch2 = xstrdup(oid_to_hex(&parent2->object.oid));
	...
	diff_tree_sha1(the_index.cache_tree->sha1,
